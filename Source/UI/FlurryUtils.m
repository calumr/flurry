...
+ (FlurryPreset*)rgbFlurryPreset {
    ...
    [fp setShortcut:@"2"];
    [fp addFlurry:[Flurry flurryWithStreams:3
                                     colour:redColorMode
                                  thickness:100.0
                                      speed:0.8]];
    [fp addFlurry:[Flurry flurryWithStreams:3
                                     colour:greenColorMode
                                  thickness:100.0
                                      speed:0.8]];
    [fp addFlurry:[Flurry flurryWithStreams:3
                                     colour:blueColorMode
                                  thickness:100.0
                                      speed:0.8]];
    return fp;
}
...
+ (FlurryPreset*)waterFlurryPreset {
    ...
    [fp setName:@"Water"];
    [fp setShortcut:@"3"];
    [fp addFlurry:[Flurry flurryWithStreams:1
                                     colour:blueColorMode
                                  thickness:100.0
                                      speed:2.0]];
    [fp addFlurry:[Flurry flurryWithStreams:1
...
+ (FlurryPreset*)fireFlurryPreset {
    ...
    [fp setShortcut:@"4"];
    [fp addFlurry:[Flurry flurryWithStreams:12
                                     colour:slowCyclicColorMode
                                  thickness:10000.0
                                      speed:0.0]];
    return fp;
}
...
+ (FlurryPreset*)psychedelicFlurryPreset {
    ...
    [fp setShortcut:@"5"];
    [fp addFlurry:[Flurry flurryWithStreams:10
                                     colour:rainbowColorMode
                                  thickness:200.0
                                      speed:2.0]];
    return fp;
}
...
+ (Flurry*)flurryWithStreams:(int)s
                      colour:(ColorModes)c
                   thickness:(float)t
                       speed:(float)sp {
...
- (id)copyWithZone:(NSZone*)zone {
    ...
    if (copy) {
        ...
        for (i = 0; i < 32; i++)
            [copy setDraws:[self shouldDrawOnScreenIndex:i randomise:NO]
                  onScreen:i];
    }
    return copy;
}
...
- (void)encodeWithCoder:(NSCoder*)coder {
    ...
    [coder encodeValueOfObjCType:@encode(ColorModes)
                              at:&flurry_info->currentColorMode];
    [coder encodeValueOfObjCType:@encode(int) at:&flurry_info->numStreams];
    [coder encodeValueOfObjCType:@encode(float)
                              at:&flurry_info->streamExpansion];
    [coder encodeValueOfObjCType:@encode(float)
                              at:&flurry_info->star->rotSpeed];
    [coder encodeObject:name];
}
...
- (id)initWithCoder:(NSCoder*)coder {
    ...
    [coder decodeValueOfObjCType:@encode(ColorModes)
                              at:&flurry_info->currentColorMode];
