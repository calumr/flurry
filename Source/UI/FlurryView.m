...
- (id)initWithFrame:(NSRect)frameRect isPreview:(BOOL)preview {
    if (self = [super initWithFrame:frameRect isPreview:preview]) {
        ...
        _glView = [[FlurryOpenGLView alloc] initWithFrame:NSZeroRect
                                              pixelFormat:format];
        ...
        randomisePreset = [[ScreenSaverDefaults
            defaultsForModuleWithName:@"Flurry"] boolForKey:RANDOM_PRESET_KEY];
    }
...
- (void)startAnimation {
    ...
    for (i = 0; i < [[[presetManager viewPreset] flurries] count]; i++) {
        Flurry* flurry =
            [[[presetManager viewPreset] flurries] objectAtIndex:i];
